...
async def add_issue_comment(
    ...
    Args:
        issue_id: Issue ID
        content: 评论内容
        parent_comment_id: 父评论 ID（可选），用于回复特定评论
        comment_type: 评论类型，可选 normal, management, handover。默认 management。
    """
    ...
    type_info = f" [{comment_type}]" if comment_type != "management" else ""
