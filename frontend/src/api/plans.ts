...
export const plansApi = {
  ...
  reject: (id: number) => api.post<Plan>(`/plans/${id}/reject`),
  listItems: (planId: number) => api.get<PlanItem[]>(`/plans/${planId}/items`),
  createItem: (planId: number, data: Partial<PlanItem>) =>
    api.post<PlanItem>(`/plans/${planId}/items`, data),
  updateItem: (planId: number, itemId: number, data: Partial<PlanItem>) =>
